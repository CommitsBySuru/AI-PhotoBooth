...
filterSelect.addEventListener('change', () => {
  ...
  updateUIState(`Filter set to ${filterSelect.options[filterSelect.selectedIndex].text}.`);
});
...
snap.addEventListener('click', async () => {
  ...
  if (countdownActive) {
    updateUIState('Countdown already running. Hold that pose.');
    return;
  }
  ...
  const countdownLength = parseInt(countdownLengthSelect.value, 10);
...
function startCamera() {
  ...
  navigator.mediaDevices.getUserMedia({ video: true })
    .then(stream => {
      ...
      updateUIState('Camera ready. Pick a filter and start the countdown.');
    })
    .catch(error => {
...
async function runCountdown(seconds) {
  ...
  updateUIState(`Countdown started: ${seconds} seconds.`);

  for (let second = seconds; second > 0; second--) {
    showCountdownValue(String(second), 'show');
    updateUIState(`Photo in ${second}...`);
    await wait(1000);
  }

  showCountdownValue('Smile!', 'flash');
  updateUIState('Smile!');
  await wait(450);
  hideCountdown();
...
function updateUIState(message) {
  ...
  snap.disabled = !cameraReady || stripComplete || countdownActive;
  retry.disabled = photoCount === 0 || countdownActive;
  download.disabled = photos.length === 0 || countdownActive;
  numPhotosSelect.disabled = countdownActive;
  countdownLengthSelect.disabled = countdownActive;
  filterSelect.disabled = countdownActive;
}

function getDefaultMessage(stripComplete) {
  if (!cameraReady) return 'Waiting for camera access.';
  if (countdownActive) return 'Countdown in progress.';
  if (stripComplete) return 'Strip complete. Download it or retry the last shot.';
  return `Ready for photo ${photoCount + 1}.`;
}

function showCountdownValue(value, className) {
  countdownOverlay.textContent = value;
  countdownOverlay.className = 'countdown-overlay';
  void countdownOverlay.offsetWidth;
  countdownOverlay.className = `countdown-overlay ${className}`;
}
